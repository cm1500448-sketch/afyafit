...
router.get('/available', authMiddleware, async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT wp.id, wp.name, wp.description, wp.difficulty, COUNT(DISTINCT wpw.id) as workout_count
  ...
  try {
    const [rows] = await db.execute(
      `SELECT upe.id, upe.program_id, upe.start_date, upe.end_date, upe.is_active,
       ...
       WHERE upe.user_id = ? AND upe.is_active = 1 ORDER BY upe.start_date DESC`,
      [req.user.id]
    );
    res.json(rows);
  } catch (err) {
...
router.post('/enroll', authMiddleware, async (req, res) => {
  try {
    ...
    const [existing] = await db.execute(
      'SELECT id FROM user_program_enrollments WHERE user_id = ? AND program_id = ? AND is_active = 1',
    ...
    await db.execute(
      'UPDATE user_program_enrollments SET is_active = 0, end_date = CURDATE() WHERE user_id = ? AND is_active = 1',
      [req.user.id]
    );
    await db.execute(
      'INSERT INTO user_program_enrollments (user_id, program_id, start_date, is_active) VALUES (?, ?, ?, 1)',
  ...
  try {
    const [programRows] = await db.execute(
      `SELECT wp.id, wp.name, wp.description, wp.difficulty, wp.created_at
       FROM weekly_programs wp WHERE wp.id = ? AND (wp.is_public = 1 OR wp.created_by = ?)`,
      [req.params.id, req.user.id]
    );
    if (programRows.length === 0) return res.status(404).json({ error: 'Program not found' });
    ...
    const [scheduleRows] = await db.execute(
      `SELECT wpw.day_of_week, wpw.order_in_day, wpw.target_sets, wpw.target_reps, wpw.target_duration_min,
       ...
       WHERE wpw.program_id = ? ORDER BY wpw.day_of_week, wpw.order_in_day`,
      [req.params.id]
    );
    ...
    scheduleRows.forEach(row => {
      ...
      schedule[day].push({
        id: row.workout_id, name: row.workout_name, description: row.workout_description,
        video_url: row.video_url, difficulty: row.workout_difficulty,
        sets: row.target_sets, reps: row.target_reps, duration: row.target_duration_min
      });
    });

    res.json({ program: programRows[0], schedule });
  } catch (err) {
...
router.get('/:id/progress', authMiddleware, async (req, res) => {
  try {
    const [enrollment] = await db.execute(
      'SELECT upe.start_date FROM user_program_enrollments upe WHERE upe.user_id = ? AND upe.program_id = ? AND upe.is_active = 1',
      [req.user.id, req.params.id]
    );
    if (enrollment.length === 0) return res.status(404).json({ error: 'Not enrolled in this program' });

    const startDate = new Date(enrollment[0].start_date);
    const daysEnrolled = Math.floor((new Date() - startDate) / (1000 * 60 * 60 * 24)) + 1;
    ...
    const [completedRows] = await db.execute(
      'SELECT COUNT(DISTINCT DATE(performed_at)) as completed_days FROM user_workout_logs WHERE user_id = ? AND program_id = ? AND performed_at >= ?',
      [req.user.id, req.params.id, enrollment[0].start_date]
    );
    ...
    res.json({ startDate: enrollment[0].start_date, daysEnrolled, totalWorkoutDays, completedDays, progressPercentage: Math.round(progressPercentage) });
  } catch (err) {
