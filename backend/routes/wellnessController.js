...
exports.getDailyStats = async (req, res) => {
  ...
  try {
    ...
    const targets = getHealthTargets(profile.date_of_birth, profile.weight_kg || 0, profile.goal_weight_kg || 0);

    if (rows.length === 0) {
      return res.json({ water_count: 0, sleep_hours: 0, mood_score: 0, steps: 0, targets });
    }

    const row = rows[0];
    res.json({
      water_count: Math.floor((row.water_ml || 0) / 250),
...
exports.updateDailyStats = async (req, res) => {
  ...
  try {
    ...
    await db.execute(
      ...
      [userId, today, waterMl, sleep_hours || null, mood_score || null,
  ...
  try {
    const [rows] = await db.execute(
      `SELECT id, description as meal_name, calories_kcal as calories, meal_date as entry_date
       FROM user_meals
       WHERE user_id = ? AND meal_date >= DATE_SUB(CURDATE(), INTERVAL 31 DAY)
...
exports.getWeeklyReport = async (req, res) => {
  ...
  try {
    const [rows] = await db.execute(
      `SELECT d.log_date, d.sleep_hours, d.water_ml,
    ...
    res.json({
      success: true,
      data: rowsWithCups,
      summary: { totalWaterCups: Math.floor(totalWaterMl / 250), avgSleep, daysTracked: rows.length }
    });
  } catch (err) {
