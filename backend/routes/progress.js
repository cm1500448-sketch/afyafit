  ...
  try {
    const [profileRows] = await db.execute(
      `SELECT weight_kg as weight, updated_at as created_at
       FROM user_profiles WHERE user_id = ? AND weight_kg IS NOT NULL
    ...
    const [wellnessRows] = await db.execute(
      `SELECT sleep_hours as sleep, notes as note, log_date as created_at
       FROM daily_wellness_logs WHERE user_id = ? AND sleep_hours IS NOT NULL
       ORDER BY log_date DESC`,
      [userId]
    );

    const logs = [
      ...profileRows.map(row => ({ id: `weight_${row.created_at}`, weight: row.weight, sleep: null, note: '', created_at: row.created_at })),
      ...wellnessRows.map(row => ({ id: `sleep_${row.created_at}`, weight: null, sleep: row.sleep, note: row.note || '', created_at: row.created_at }))
...
router.post('/add-log', authMiddleware, async (req, res) => {
  ...
  try {
    if (weight) {
      await db.execute('UPDATE user_profiles SET weight_kg = ? WHERE user_id = ?', [weight, userId]);
    }
