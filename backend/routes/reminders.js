...
router.get('/', authMiddleware, async (req, res) => {
  try {
    const [reminders] = await db.execute(
      `SELECT * FROM reminders WHERE user_id = ? AND is_active = TRUE ORDER BY scheduled_time ASC`,
      [req.user.id]
    );
    res.json(reminders);
  } catch (err) {
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `INSERT INTO reminders (user_id, reminder_type, title, message, scheduled_time) VALUES (?, ?, ?, ?, ?)`,
...
router.get('/check-exercise', authMiddleware, async (req, res) => {
  try {
    ...
    const [todayWorkouts] = await db.execute(
      'SELECT COUNT(*) as count FROM user_workout_logs WHERE user_id = ? AND DATE(performed_at) = ?',
      [userId, today]
    );

    const [lastWorkout] = await db.execute(
      'SELECT MAX(DATE(performed_at)) as last_date FROM user_workout_logs WHERE user_id = ?',
      [userId]
    );

    const [reminderSent] = await db.execute(
      'SELECT COUNT(*) as count FROM exercise_reminder_logs WHERE user_id = ? AND DATE(sent_at) = ?',
      [userId, today]
    );
    ...
    res.json({
      ...
      last_workout_date: lastWorkout[0]?.last_date,
...
router.post('/mark-sent', authMiddleware, async (req, res) => {
  try {
    ...
    await db.execute(
      'INSERT INTO exercise_reminder_logs (user_id, reminder_id) VALUES (?, ?)',
