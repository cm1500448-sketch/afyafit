...
const Fitness = ({ onCompleteWorkout }) => {
  ...
  useEffect(() => {
    if (completionRate === 100 && todaysPlan.exercises.length > 0) {
      ...
      (function frame() {
        confetti({ particleCount: 3, angle: 60, spread: 55, origin: { x: 0 }, colors: [todaysPlan.color, '#ffffff'] });
        confetti({ particleCount: 3, angle: 120, spread: 55, origin: { x: 1 }, colors: [todaysPlan.color, '#ffffff'] });
  ...
  return (
    <div className="fitness-container">
      {activeWorkout && (
        <WorkoutPlayer workout={activeWorkout} onClose={() => setActiveWorkout(null)} />
      )}
        ...
        {!showAllLibrary && todaysPlan.exercises.length > 0 && (
          <div className="progress-card">
            ...
            <div className="progress-bar-bg">
              <div className="progress-bar-fill" style={{ width: `${completionRate}%`, backgroundColor: todaysPlan.color }} />
            </div>
          </div>
        )}
      </header>
      ...
      <section className="fitness-section">
        <WeeklySchedule days={days} todayIndex={todayIndex} todaysPlan={todaysPlan} />
      </section>
      ...
      <section className="fitness-section main-content">
        ...
        <div className="exercise-list">
          {showAllLibrary ? (
            <ExerciseLibrary exercises={exerciseLibrary} searchTerm={searchTerm} favorites={favorites} onToggleFavorite={toggleFavorite} onWatchTutorial={handleWatchTutorial} />
          ) : (
            <TodaysPlan plan={todaysPlan} completedExercises={completedExercises} favorites={favorites} saveStatus={saveStatus} onToggleExercise={toggleExercise} onToggleFavorite={toggleFavorite} onWatchTutorial={handleWatchTutorial} onFinishWorkout={() => handleFinishWorkout(onCompleteWorkout)} />
          )}
        </div>
      </section>
    </div>
