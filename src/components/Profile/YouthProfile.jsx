...
const YouthProfile = () => {
  ...
  const fetchCoachRequest = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5000/api/coach-requests/my-request', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
        // API returns { request: {...} } — unwrap it
        const requestObj = data.request || data;
        setCoachRequest(requestObj);
  ...
  const handleUpdateLevel = async (newLevel) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch('http://localhost:5000/api/auth/update-level', {
        method: 'PUT',
        headers: { 
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5000/api/parent/generate-code', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleCoachRequestSuccess = () => {
    fetchCoachRequest();
    setMessage('Coach request submitted successfully!');
    setTimeout(() => setMessage(''), 3000);
  };

  const handleCoachRequestCancel = () => {
    fetchCoachRequest();
    setMessage('Coach request cancelled.');
    setTimeout(() => setMessage(''), 3000);
  };
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => setMessage(''), 3000);
        // Refetch profile from DB to keep UI in sync
    ...
    <div className="profile-container">
      ...
      <div className="profile-card parent-link-card">
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          />
          <button className="update-btn" onClick={handleGenerateCode}>
      ...
      <div className="profile-card coach-request-card">
        ...
        {!hasCoachRequest ? (
          ...
          <CoachRequestStatus 
            ...
            onCancel={handleCoachRequestCancel} 
          />
        )}
      </div>
